...
{
    // create locations
    // create vars that store indexes of locations
    ...
    this->locations.push_back(maryIdemaPewLibrary);
    int maryIdemaPewLibraryIndex = this->locations.size() - 1;
    ...
    this->locations.push_back(mackinacHall);
    int mackinacHallIndex = this->locations.size() - 1;
    ...
    this->locations.push_back(lakerVillage);
    int lakerVillageIndex = this->locations.size() - 1;

    // Blue Connection
    // 2 floors
    Location blueConnection("Blue Connection",
                            "A common area with a small food-court and a convenience store.");
    this->locations.push_back(blueConnection);
    int blueConnectionIndex = this->locations.size() - 1;
    ...
    this->locations.push_back(kirkhofCenterParkingLot);
    int kirkhofCenterParkingLotIndex = this->locations.size() - 1;

    // Clock Tower
    Location clockTower("Clock Tower",
                        "A large clock tower, the center of campus.");
    this->locations.push_back(clockTower);
    int clockTowerIndex = this->locations.size() - 1;
    ...
    now decrepit.");
    this->locations.push_back(kirkhofCenter);
    int kirkhofCenterIndex = this->locations.size() - 1;

    // Fresh
    // 2 floors
    Location fresh("Fresh",
                   "A buffet-style cafeteria.");
    this->locations.push_back(fresh);
    int freshIndex = this->locations.size() - 1;
    ...
    this->locations.push_back(marketplace);
    int marketplaceIndex = this->locations.size() - 1;

    // Henry Hall
    // 2 floors?
    Location henryHall("Henry Hall",
                       "This building contains several computer labs.");
    this->locations.push_back(henryHall);
    int henryHallIndex = this->locations.size() - 1;

    // Woods
    Location woods("Woods",
                   "A small patch of woods behind campus. The elf lives here.");
    this->locations.push_back(woods);
    int woodsIndex = this->locations.size() - 1;
    ...
    locations[lakerVillageIndex].add_ref_location("East", locations[blueConnectionIndex]);
    locations[lakerVillageIndex].add_ref_location("Northeast", locations[kirkhofCenterParkingLotIndex]);

    // Blue Connection:
    // West: Laker Village
    // North: Kirkhof Center Parking Lot
    locations[blueConnectionIndex].add_ref_location("West", locations[lakerVillageIndex]);
    locations[blueConnectionIndex].add_ref_location("North", locations[kirkhofCenterParkingLotIndex]);
    ...
    locations[kirkhofCenterParkingLotIndex].add_ref_location("South", locations[blueConnectionIndex]);
    locations[kirkhofCenterParkingLotIndex].add_ref_location("Southwest", locations[lakerVillageIndex]);
    locations[kirkhofCenterParkingLotIndex].add_ref_location("North", locations[maryIdemaPewLibraryIndex]);
    locations[kirkhofCenterParkingLotIndex].add_ref_location("Northeast", locations[kirkhofCenterIndex]);

    // Mary Idema Pew Library:
    // South: Kirkhof Center Parking Lot
    // East: Kirkhof Center
    // Northeast: Clock Tower
    locations[maryIdemaPewLibraryIndex].add_ref_location("South", locations[kirkhofCenterParkingLotIndex]);
    locations[maryIdemaPewLibraryIndex].add_ref_location("East", locations[kirkhofCenterIndex]);
    locations[maryIdemaPewLibraryIndex].add_ref_location("Northeast", locations[clockTowerIndex]);

    // Kirkhof Center:
    // Southwest: Kirkhof Center Parking Lot
    // West: Mary Idema Pew Library
    // North: Clock Tower
    locations[kirkhofCenterIndex].add_ref_location("Southwest", locations[kirkhofCenterParkingLotIndex]);
    locations[kirkhofCenterIndex].add_ref_location("West", locations[maryIdemaPewLibraryIndex]);
    locations[kirkhofCenterIndex].add_ref_location("North", locations[clockTowerIndex]);

    // Clock Tower:
    // Southwest: Mary Idema Pew Library
    // South: Kirkhof Center
    // North: Henry Hall
    locations[clockTowerIndex].add_ref_location("Southwest", locations[maryIdemaPewLibraryIndex]);
    locations[clockTowerIndex].add_ref_location("South", locations[kirkhofCenterIndex]);
    locations[clockTowerIndex].add_ref_location("North", locations[henryHallIndex]);

    // Henry Hall:
    // South: Clock Tower
    // Northwest: Marketplace
    // Northeast: Fresh
    locations[henryHallIndex].add_ref_location("South", locations[clockTowerIndex]);
    locations[henryHallIndex].add_ref_location("Northwest", locations[marketplaceIndex]);
    locations[henryHallIndex].add_ref_location("Northeast", locations[freshIndex]);

    // Marketplace:
    // Southest: Henry Hall
    locations[marketplaceIndex].add_ref_location("Southeast", locations[henryHallIndex]);

    // Fresh:
    // Southwest: Henry Hall
    // North: Mackinac Hall
    // East: Woods
    locations[freshIndex].add_ref_location("Southwest", locations[henryHallIndex]);
    locations[freshIndex].add_ref_location("North", locations[mackinacHallIndex]);
    locations[freshIndex].add_ref_location("East", locations[woodsIndex]);

    // Mackinac Hall:
    // South: Fresh
    // Southeast: Woods
    locations[mackinacHallIndex].add_ref_location("South", locations[freshIndex]);
    locations[mackinacHallIndex].add_ref_location("Southeast", locations[woodsIndex]);

    // Woods:
    // West: Fresh
    // Northwest: Mackinac Hall
    locations[woodsIndex].add_ref_location("West", locations[freshIndex]);
    locations[woodsIndex].add_ref_location("Northwest", locations[mackinacHallIndex]);
    ...
    Item connectionSandwich = Item("Stingin' Honey Chicken Sandwich",
                                   ...
                                   55, 1);
    this->locations[blueConnectionIndex].add_ref_item(connectionSandwich);
    Item lakerBowl = Item("Laker Bowl",
                          "The legendary Laker Bowl, popcorn chicken, mashed potatoes, corn\
    , and gravy",
                          50, 1.5);
    this->locations[blueConnectionIndex].add_ref_item(lakerBowl);
    Item italianSub = Item("Italian Sub",
                           "A classic Italian sub, with ham, salami, pepperoni,\
    and provolone.",
                           45, 1);
    this->locations[blueConnectionIndex].add_ref_item(italianSub);
    Item energyBarConnection = Item("Energy Bar",
                                    "A protein bar.", 10, .5);
    this->locations[blueConnectionIndex].add_ref_item(energyBarConnection);
    Item bagOfChipsConnection = Item("Barbecue Chips",
                                     "A bag of delicious barbecue chips.", 10, .5);
    this->locations[blueConnectionIndex].add_ref_item(bagOfChipsConnection);
    Item coffeeConnection = Item("Coffee",
                                 "The lifeblood of college students.", 10, .5);
    this->locations[blueConnectionIndex].add_ref_item(coffeeConnection);

    // fresh
    Item freshPizza = Item("Pizza",
                           "A slice of cheese pizza.", 50, 1);
    this->locations[freshIndex].add_ref_item(freshPizza);
    Item freshBurger = Item("Burger",
                            "A cold cheeseburger.", 40, 1);
    this->locations[freshIndex].add_ref_item(freshBurger);
    Item freshSalad = Item("Salad",
                           "A caesar salad.", 25, 1);
    this->locations[freshIndex].add_ref_item(freshSalad);
    Item coffeeFresh = Item("Coffee",
                            "The lifeblood of college students.", 10, .5);
    this->locations[freshIndex].add_ref_item(coffeeFresh);

    // marketplace
    Item marketplaceRamen = Item("Ramen",
                                 "A cup of ramen.", 45, .5);
    this->locations[marketplaceIndex].add_ref_item(marketplaceRamen);
    Item marketplaceSushi = Item("Sushi",
                                 "A tray of old sushi. It's probably still good...", 20, 1);
    this->locations[marketplaceIndex].add_ref_item(marketplaceSushi);
    Item coffeeMarketplace = Item("Coffee",
                                  "The lifeblood of college students.", 10, .5);
    this->locations[marketplaceIndex].add_ref_item(coffeeMarketplace);

    // mackinacHall
    Item energyBarMac = Item("Energy Bar",
                             "A protein bar.", 10, .5);
    this->locations[mackinacHallIndex].add_ref_item(energyBarMac);
    Item bagOfChipsMac = Item("Barbecue Chips",
                              "A bag of delicious barbecue chips.", 10, .5);
    this->locations[mackinacHallIndex].add_ref_item(bagOfChipsMac);
    Item coffeeMac = Item("Coffee",
                          "The lifeblood of college students.", 10, .5);
    this->locations[mackinacHallIndex].add_ref_item(coffeeMac);

    // kirhofCenter
    Item trailMixKirkhof = Item("Trail Mix",
                                "A bag of trail mix. Contains peanuts.", 10, .5);
    this->locations[kirkhofCenterIndex].add_ref_item(trailMixKirkhof);
    Item coffeeKirkhof = Item("Coffee",
                              "The lifeblood of college students.", 10, .5);
    this->locations[kirkhofCenterIndex].add_ref_item(coffeeKirkhof);

    // clockTower
    Item trailMixClockTower = Item("Trail Mix",
                                   "A bag of trail mix. Contains peanuts.", 10, .5);
    this->locations[clockTowerIndex].add_ref_item(trailMixClockTower);

    // henryHall
    Item bagOfChipsHenry = Item("Barbecue Chips",
                                "A bag of delicious barbecue chips.", 10, .5);
    this->locations[henryHallIndex].add_ref_item(bagOfChipsHenry);
    Item trailMixHenry = Item("Trail Mix",
                              "A bag of trail mix. Contains peanuts.", 10, .5);
    this->locations[henryHallIndex].add_ref_item(trailMixHenry);
    Item coffeeHenry = Item("Coffee",
                            "The lifeblood of college students.", 10, .5);
    this->locations[henryHallIndex].add_ref_item(coffeeHenry);

    // lakerVillage
    Item energyBarVillage = Item("Energy Bar",
                                 "A protein bar.", 10, .5);
    this->locations[lakerVillageIndex].add_ref_item(energyBarVillage);
    Item bagOfChipsVillage = Item("Barbecue Chips",
                                  "A bag of delicious barbecue chips.", 10, .5);
    this->locations[lakerVillageIndex].add_ref_item(bagOfChipsVillage);

    // kirkhofCenterParkingLot
    Item bagOfChipsParking = Item("Barbecue Chips",
                                  "A bag of delicious barbecue chips.", 10, .5);
    this->locations[kirkhofCenterParkingLotIndex].add_ref_item(bagOfChipsParking);
    Item trailMixParking = Item("Trail Mix",
                                "A bag of trail mix. Contains peanuts.", 10, .5);
    this->locations[kirkhofCenterParkingLotIndex].add_ref_item(trailMixParking);

    // Mary Idema Pew Library
    Item bagOfChipsLibrary = Item("Barbecue Chips",
                                  "A bag of delicious barbecue chips.", 10, .5);
    this->locations[maryIdemaPewLibraryIndex].add_ref_item(bagOfChipsLibrary);
    Item trailMixLibrary = Item("Trail Mix",
                                "A bag of trail mix. Contains peanuts.", 10, .5);
    this->locations[maryIdemaPewLibraryIndex].add_ref_item(trailMixLibrary);
    Item coffeeLibrary = Item("Coffee",
                              "The lifeblood of college students.", 10, .5);
    this->locations[maryIdemaPewLibraryIndex].add_ref_item(coffeeLibrary);
    ...
    this->locations[henryHallIndex].add_ref_npc(vendingMachineStocker);
    // Laker Village
    ...
    this->locations[woodsIndex].add_ref_npc(elf);
...
Location& Game::random_location()
{
    ...
    std::mt19937 gen(rd());
    std::uniform_int_distribution<> dis(0, this->ref_locations.size() - 1);
    // get random index
    int random_index = dis(gen);
    // return location at random index
    return this->ref_locations[random_index];
...
void Game::talk(std::vector<std::string> target)
{
    ...
    while (npc_iterator != npcs.end())
    {
        ...
        if (npc_iterator->getName() == target_npc)
        {
            // get message from npc
            std::string message = npc_iterator->getCurrentMessage();
            // print message
...
void Game::meet(std::vector<std::string> target)
{
    ...
    while (npc_iterator != npcs.end())
    {
        ...
        if (npc_iterator->getName() == target_npc)
        {
            ...
            std::string description = npc_iterator->getDescription();
            // print description
...
void Game::take(std::vector<std::string> target)
{
    ...
    while (item_iterator != items.end())
    {
        ...
        if (item_iterator->getName() == target_npc)
        {
            // add to user inventory
            this->inventory.push_back(*item_iterator);
            // remove item from room
            this->current_location.remove_ref_item(*item_iterator);
            // add weight to user's carry weight
            this->current_weight += item_iterator->getWeight();
        }
    }
}
...
void Game::give(std::vector<std::string> target)
{
    ...
    {
        // check if item name matches target
        if (item_iterator->getName() == target_item)
        {
            // add to location inventory
            this->current_location.add_ref_item(*item_iterator);
            ...
            if (this->current_location.get_name() == "Woods")
            {
                ...
                if (item_iterator->getCalories() > 0)
                {
                    // subtract calories from calories needed
                    this->calories_needed -= item_iterator->getCalories();
...
void Game::go(std::vector<std::string> target)
{
    ...
    else
    {
        ...
        std::map<std::string, std::reference_wrapper<Location>> neighbors = this->current_location.get_ref_locations();
        // neighbor iterator
...
// get locations
std::vector<Location> Game::get_locations() const
{
    return this->locations;
}
std::vector<std::reference_wrapper<Location>> Game::get_ref_locations()
{
    return this->ref_locations;
}
